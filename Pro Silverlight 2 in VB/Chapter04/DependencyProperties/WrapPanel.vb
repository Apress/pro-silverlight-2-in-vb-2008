Public Class WrapPanel
    ...
    Public Shared ReadOnly OrientationProperty As DependencyProperty = DependencyProperty.Register( _
      "Orientation", GetType(Orientation), GetType(WrapPanel), New PropertyMetadata(Orientation.Horizontal))

    Public Shared LineBreakBeforeProperty As DependencyProperty = _
      DependencyProperty.RegisterAttached("LineBreakBefore", _
        GetType(Boolean), GetType(WrapPanel), Nothing)
    ...
    Protected Overrides Function MeasureOverride(ByVal constraint As Size) As Size
        ...
        For Each element As UIElement In Me.Children
            ...
            If (currentLineSize.Width + desiredSize.Width > constraint.Width) OrElse (WrapPanel.GetLineBreakBefore(element)) Then
    ...
    Protected Overrides Function ArrangeOverride(ByVal arrangeBounds As Size) As Size
        ...
        For Each element As UIElement In Me.Children
            ...
            ' Check if the element fits in the line, if given its desired size.
            If (currentLineSize.Width + desiredSize.Width > arrangeBounds.Width) OrElse (WrapPanel.GetLineBreakBefore(element)) Then
                ' Switch to a new line because space has run out.
